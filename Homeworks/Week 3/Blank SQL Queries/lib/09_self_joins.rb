...
def craiglockhart_to_sighthill
  ...
  execute(<<-SQL)
    SELECT DISTINCT
      start.num, start.company, transfer_stop.name, finish.num, finish.company
    FROM
      routes start
      JOIN
        stops start_stop ON start.stop_id = start_stop.id
      JOIN
       routes to_transfer ON (start.company = to_transfer.company
      ...
      JOIN
        routes finish ON (from_transfer.company = finish.company
        AND from_transfer.num = finish.num)
      JOIN
        stops finish_stop ON finish.stop_id = finish_stop.id
    WHERE
      start_stop.name = 'Craiglockhart'
      AND
      finish_stop.name = 'Sighthill'
  SQL
end
